...
class TestBedModel < GardenTest
  ...
  def test_canvas_points_array_nil_when_unset
    bed = Bed.create(name: "B1", bed_type: "raised", garden_id: @garden.id)
    assert_equal [], bed.canvas_points_array
  end

  def test_canvas_points_array_roundtrip
    pts = [[10, 20], [30, 40], [50, 10]]
    bed = Bed.create(name: "B2", bed_type: "raised", garden_id: @garden.id)
  ...
  def test_placed_false_when_canvas_x_nil
    bed = Bed.create(name: "B3", bed_type: "raised", garden_id: @garden.id)
    refute bed.placed?
  end

  def test_placed_true_when_canvas_x_set
    bed = Bed.create(name: "B4", bed_type: "raised",
                     canvas_x: 10.0, canvas_y: 20.0, garden_id: @garden.id)
    assert bed.placed?
  end

  def test_polygon_false_for_rectangle
    bed = Bed.create(name: "B5", bed_type: "raised", garden_id: @garden.id)
    refute bed.polygon?
  end

  def test_polygon_true_when_points_set
    bed = Bed.create(name: "B6", bed_type: "raised", garden_id: @garden.id)
